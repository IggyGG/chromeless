...
**Status: FIXED and VERIFIED LIVE, 2026-09-08.** `tests/interactive --only
uploads` is **9/9** on guest `cr7727-f9deaaa7efc6`: the page receives a real
`File`, with the original filename, the right size, and contents matching
byte for byte. The guest log ends `complete: 27 bytes, sha ok, attached to
the page's file chooser`.

Kept rather than deleted, per this directory's contract, because three
theories were refuted along the way and the refutations are the expensive
part. Reproduced against the live standalone
...
## THE ACTUAL CAUSE, 2026-09-08 — a use-after-move in the reply lambda

Two theories preceded it and **both were wrong**; both are left standing above
because "this was refuted" is the half of a diagnosis that vanishes if only
the winning answer gets written down.

The instrumentation settled it in one run:

```
CV2-UPLOAD: WriteFile -> ok, 27 bytes; file is now 27 at .../Uploads/<id>__...
CV2-UPLOAD: finalising <id>; hashing .../Uploads/<id>__...
CV2-UPLOAD: upload <id> failed: could not read the file back
```

The write succeeded. The file was **27 bytes on disk**. And the ERROR line
added to `HashFile`'s read-failure path — `grep -c 'ReadFileToString FAILED'`
— printed **zero** times, while the other two new lines printed, so the probe
worked and the read simply never failed.

Both halves succeeded and the caller still saw failure:
...
argument evaluation order unspecified.** Argument 3 won, so `ok` was computed
from a moved-from empty string: every upload reported failure with the file
correct and complete on disk.

Fixed by computing the flag into a named local first (`f9deaaa`).

### Why it cost five rolls: the message named a cause that never happened

`"could not read the file back"` is what `OnFinalised` says when `ok` is
false. The read had never failed once. That single misleading string sent the
diagnosis through the filesystem, permissions, the task runner, and a genuine
`end`/write race — the last of which was real and its fix stands, but it was
not this.

The code now reports `hash_failed` and says only what it knows, with the read
logging its own failure separately.

### What is now cheap that was not

| added | catches |
| --- | --- |
| `make lint-cxx-use-after-move` | a move and a read of one name in a single argument list — the unsequenced case only |
| `make lint-cxx-orphan` | a method declared in a `.h` and defined in no `.cc` (this cost a separate roll) |
| per-frame + per-write logging on the files channel | which of write / read / neither actually failed |
