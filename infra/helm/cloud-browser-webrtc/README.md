# cloud-browser-webrtc Helm chart

Phase 1 deployment of cloud-browser-webrtc onto Kubernetes. Wraps
...
helm install cloud-browser-webrtc \
    oci://ghcr.io/iggy/cloud-browser-webrtc/helm/cloud-browser-webrtc \
    --version 0.1.0 \
...
| Default BrowserSessionPool with cb-chromium + cb-metrics-sidecar containers | `defaultPool.enabled: true` (default) |
...
- **Seccomp**: the chart references a `Localhost: cb-chromium.json`
  seccomp profile. Each kubelet must have
  `/var/lib/kubelet/seccomp/cb-chromium.json` installed before the
...
```sh
cd infra/helm/cloud-browser-webrtc
helm lint .
helm template . \
...
error when omitted; they're the same `cb.requireValue` helper used
across the chart.

## Upgrade

```sh
helm upgrade cloud-browser-webrtc ... -f my-values.yaml
```
...
```sh
helm uninstall cloud-browser-webrtc --namespace cloud-browser-webrtc
