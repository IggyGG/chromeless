#!/usr/bin/env bash
#
# infra/k8s/cb-prod/install.sh — install / upgrade the cloud-browser-webrtc
# Helm release on the Triform K8s cluster (T115).
#
# Usage:
#   CB_IMAGE_TAG=<tag> [CB_SECRETS_FILE=<path>] [DRY_RUN=1] install.sh [helm-args...]
#
# Required:
#   CB_IMAGE_TAG     — image tag for every cb-* image. Set to the
...
#
# Optional:
#   CB_SECRETS_FILE  — path to a values overlay carrying the auth
#                       pubkey + TURN shared secret + TURN URLs.
#                       NOT committed to git. See README.md /
#                       triform-deploy.md for the schema. Defaults
#                       to ~/.cb-secrets-triform.yaml if present.
#   DRY_RUN          — set to "1" to render + dry-run-server only
#                       (no actual install).
#   CB_RELEASE_NAME  — Helm release name. Default: cb.
#   CB_NAMESPACE     — target namespace. Default: cb-prod.
...
#   3. Validates CB_IMAGE_TAG, CB_SECRETS_FILE.
...
CHART_DIR="${REPO_ROOT}/infra/helm/cloud-browser-webrtc"
VALUES_FILE="${SCRIPT_DIR}/values-triform.yaml"

CB_RELEASE_NAME="${CB_RELEASE_NAME:-cb}"
CB_NAMESPACE="${CB_NAMESPACE:-cb-prod}"
CB_SECRETS_FILE="${CB_SECRETS_FILE:-${HOME}/.cb-secrets-triform.yaml}"
...
[[ -n "${CB_IMAGE_TAG:-}" ]] || die "CB_IMAGE_TAG is required (e.g. m147-roll1-abc1234)"
...
log "applying namespace ${CB_NAMESPACE}"
kubectl apply -f - <<EOF
apiVersion: v1
kind: Namespace
metadata:
...
if ! kubectl get secret -n "${CB_NAMESPACE}" registry-pull >/dev/null 2>&1; then
    log "copying registry-pull from default ns into ${CB_NAMESPACE}"
    kubectl get secret -n default registry-pull -o yaml \
        | sed "s/namespace: default/namespace: ${CB_NAMESPACE}/" \
        | kubectl apply -f -
fi
...
if [[ -f "${CB_SECRETS_FILE}" ]]; then
    log "secrets overlay: ${CB_SECRETS_FILE}"
    HELM_VALUES_ARGS+=("-f" "${CB_SECRETS_FILE}")
else
    log "WARN: no CB_SECRETS_FILE at ${CB_SECRETS_FILE}; relying on --set passes from caller"
fi

# Per-image tag --set arguments. CB_IMAGE_TAG goes to every image
...
for img in chromium signaling controller turnIssuer metricsSidecar inputBridge cursorWatcher clipboardBridge; do
    HELM_TAG_ARGS+=("--set" "images.${img}.tag=${CB_IMAGE_TAG}")
done

log "release        : ${CB_RELEASE_NAME}"
log "namespace      : ${CB_NAMESPACE}"
log "image tag      : ${CB_IMAGE_TAG}"
log "values         : ${VALUES_FILE}"
log "secrets overlay: ${CB_SECRETS_FILE}${CB_SECRETS_FILE:+ (exists=$([[ -f ${CB_SECRETS_FILE} ]] && echo yes || echo no))}"
log "extra helm args: $*"
...
helm template "${CB_RELEASE_NAME}" "${CHART_DIR}" \
    "${HELM_VALUES_ARGS[@]}" \
    "${HELM_TAG_ARGS[@]}" \
    "$@" \
    > /tmp/cb-rendered.yaml 2> /tmp/cb-render.err
preview_status=$?
if [[ ${preview_status} -ne 0 ]]; then
    cat /tmp/cb-render.err >&2
    die "helm template failed (rc=${preview_status}); fix values and retry"
fi
preview_lines=$(wc -l < /tmp/cb-rendered.yaml | tr -d ' ')
log "rendered ${preview_lines} lines to /tmp/cb-rendered.yaml; head:"
head -30 /tmp/cb-rendered.yaml
log "... (full output at /tmp/cb-rendered.yaml)"
...
HELM_CMD=(
    helm upgrade --install "${CB_RELEASE_NAME}" "${CHART_DIR}"
...
kubectl -n "${CB_NAMESPACE}" run cb-smoke-signaling-$$ \
    --image=curlimages/curl:8.10.1 \
    --restart=Never \
    --rm -i --quiet \
    --command -- \
    curl -fsS --max-time 10 \
    "http://signaling.${CB_NAMESPACE}.svc.cluster.local:8080/healthz"
echo
...
kubectl -n "${CB_NAMESPACE}" get pods -l app.kubernetes.io/component=session 2>&1 || true
