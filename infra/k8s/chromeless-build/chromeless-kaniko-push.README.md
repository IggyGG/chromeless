...
Apply it explicitly after a successful `chromeless-build-${variant}` Job:
...
`${CHROMELESS_WORK_ROOT}/artifacts/context/` (typically
`/mnt/data/chromeless-build-t{2,5}/chromium-src/artifacts/context/`) but
...
1. **Wait for chromeless-build-${variant} to reach Succeeded.**
   ```bash
   kubectl get -n chromeless-build job chromeless-build-x264 -o jsonpath=
...
2. **Confirm the build context is staged on the right node.**
   ...
   ssh triform-2 ls /mnt/data/chromeless-build-t2/chromium-src/artifacts/context
   # Expect: Dockerfile  IMAGE_TAG  cloud_browser_worker  launch-chromeless.sh  supervisord.conf

   # nvenc (triform-5):
   ssh triform-5 ls /mnt/data/chromeless-build-t5/chromium-src/artifacts/context
   ```
...
4. **Apply the Job.** If it's already applied from a prior cycle,
   ...
   ```bash
   kubectl -n chromeless-build delete job chromeless-kaniko-push --ignore-not-found
   kubectl apply -f infra/k8s/chromeless-build/chromeless-kaniko-push.yaml
   kubectl logs -n chromeless-build job/chromeless-kaniko-push -f
   ```

5. **Verify the tag landed in the registry.**
   ```bash
   curl -sf -u "${FORGEJO_USERNAME}:${FORGEJO_PASSWORD}" \
     https://registry.triform.cloud/v2/chromeless/chromeless/manifests/cr7727-sw \
...
|---------|----------|----------------|---------------|
| sw / x264 (default) | `triform-2` | `/mnt/data/chromeless-build-t2/chromium-src/artifacts/context` | `cr7727-sw` |
| vaapi | `triform-2` | `/mnt/data/chromeless-build-t2/chromium-src/artifacts/context` | `cr7727-vaapi` |
...
- `spec.template.spec.containers[0].env[name=CHROMELESS_KANIKO_TAG].value`
  ...
  template:
    spec:
      ...
      volumes:
        - name: workspace
          hostPath:
            path: /mnt/data/chromeless-build-t5/chromium-src/artifacts/context
            type: Directory
```
...
2. `crictl tag` the loaded image to `chromeless:cr7727-${variant}`
...
- `infra/k8s/chromeless-build/build-job-x264.yaml` — produces the build
  context this Job consumes (Step 9 of chromeless-build.sh).
- `infra/k8s/chromeless-build/build-job-vaapi.yaml` — same, vaapi profile.
- `infra/k8s/chromeless-build/build-job-nvenc.yaml` — same, nvenc profile,
  triform-5.
- `build/chromeless-build.sh` — the Step 9 staging logic that produces
  `${CHROMELESS_WORK_ROOT}/artifacts/context/`.
