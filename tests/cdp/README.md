...
A pytest suite that drives the chromeless binary directly via Chrome
...
crashes the chromeless binary (tracked separately as a follow-up to
the `fcbeb16` embedder fix). Leaking contexts is harmless — the
chromeless-cdp-validation pod runs once per build and is GC'd immediately.
...
```sh
# 1. Port-forward chromeless-browserless to your workstation.
kubectl -n triform-wtf port-forward svc/chromeless-browserless 9222:9222 &
...
CHROMELESS_URL=http://localhost:9222 pytest tests/cdp/ -v
```
...
`build/chromeless-build.sh` Step 10 (`cdp validation`) runs after the kaniko
sidecar has pushed the image. It applies
`infra/k8s/tests/chromeless-cdp-validation.yaml`, which schedules a
`python:3.11-slim` Job in the `chromeless-tests` namespace that mounts this
directory and runs `pytest tests/cdp/`. The image tag of the just-pushed
binary is passed to the Job via the `CHROMELESS_TEST_IMAGE_TAG` env var (NOT sed
...
`chromeless:latest` and physics will continue to use whatever the prior
...
hand), run chromeless-build.sh up through Step 9 with whatever override the
build host supports — but the full `bash build/chromeless-build.sh` invocation
