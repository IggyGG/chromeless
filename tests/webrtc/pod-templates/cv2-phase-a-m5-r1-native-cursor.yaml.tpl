...
# harness (phase-a-m5-r1-native-cursor.mjs) that does NOT fire a cursor stimulus —
...
#   __REV_TAG__     — short rev label, e.g. `rv6-native-cursor`
...
metadata:
  ...
  labels:
    ...
    app.kubernetes.io/variant: cv2-m5-r1-native-cursor
    ...
    - name: bootstrap
      ...
      command:
        ...
        - |
          ...
          cp /harness/phase-a-m5-r1-native-cursor.mjs /workspace/tests/webrtc/
    ...
    - name: client-test-driver
      ...
      command:
        ...
        - |
          ...
          if node phase-a-m5-r1-native-cursor.mjs 2>&1 | tee /tmp/out/m5-r1-native-cursor.log; then
            HARNESS_EXIT=0
            echo "M5 R1 DOM probe: COMPLETE (structured results in log)"
          else
            HARNESS_EXIT=$?
            echo "M5 R1 DOM probe: FAIL (exit=${HARNESS_EXIT}) — see /tmp/out/m5-r1-native-cursor.log for diagnostic"
          fi
          echo "m5_r1_dom_probe_exit_code=${HARNESS_EXIT}" > /tmp/out/m5-r1-native-cursor-exit-code
